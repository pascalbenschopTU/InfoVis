...
class GraphWrapper{
    ...
    plotDataGraph(){
        ...
        // }
        this.plotDataThresholdLine(160)
    ...
    calculateAxisScaling(data){
        // scale data to axis
        this.xScale = d3.scaleLinear().domain([d3.min(data, (d) => d[0]), d3.max(data, (d) => d[0])]).range([0, width]);
        this.yScale = d3.scaleLinear().domain([0, d3.max(data, (d) => parseFloat(d[1]))]).range([height, 0]);
    ...
    addMouseHover(data){
        ...
        mouse_g.append('rect').attr('width', 2).attr('x',-4).attr('y', 10).attr('height', height).attr('fill', 'lightgray');
